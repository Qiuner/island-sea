...
export const ISLAND_PROPS: ((group: THREE.Group, ctx: PropCtx) => PropUpdate)[] = [
  (group) => {
  ...
  (group, ctx) => {
    ...
    return (sim, nightK) => {
      const flicker = 0.55 + 0.25 * Math.sin(sim * 8) + 0.15 * Math.sin(sim * 13.7)
      ;(glow.material as THREE.SpriteMaterial).opacity = 0.4 + 0.25 * flicker + nightK * 0.35
