...
export function createPierLantern(group: THREE.Group, radius: number, rng: () => number) {
  const pierAngle = rng() * Math.PI * 2
  const pier = new THREE.Group()
  const plankA = new THREE.MeshLambertMaterial({ color: '#a06a3e', flatShading: true })
  const plankB = new THREE.MeshLambertMaterial({ color: '#8f5d35', flatShading: true })
  const legMat = new THREE.MeshLambertMaterial({ color: '#6e4a2a', flatShading: true })
  const nPlanks = 6
...
export function createFogSprites(
  ...
  for (let i = 0; i < 5; i++) {
    ...
    fog.position.set(Math.cos(a) * radius * 0.85, 2.5 + rng() * 3.5, Math.sin(a) * radius * 0.85)
    fog.scale.setScalar(radius * (1.1 + rng() * 0.5))
    fog.userData.orbit = { a, r: radius * 0.85, speed: 0.05 + rng() * 0.05, y: fog.position.y }
    fogSprites.push(fog)
    group.add(fog)
  }
  return fogSprites
...
export function createChestDisplay(group: THREE.Group, def: IslandDef, radius: number, topY: number) {
  ...
  def.projects.forEach((project, idx) => {
    ...
    const angle = def.projects.length > 1 ? (idx / def.projects.length) * Math.PI * 2 : 0
    const ringRadius = def.projects.length > 1 ? radius * 0.55 : 0
    const baseY = topY + 6
    chest.position.set(Math.cos(angle) * ringRadius, baseY, Math.sin(angle) * ringRadius)
