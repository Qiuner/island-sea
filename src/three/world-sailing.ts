...
interface SailingContext {
  ...
  dt: number
  boundaryK: number
  ship: { pos: THREE.Vector3; heading: number; speed: number; forward: THREE.Vector3 }
...
export function updateSailing(ctx: SailingContext): SailingResult {
  ...
  if (ctx.boundaryK > 0.01) {
    ...
    ctx.ship.heading += wrapAngle(toCenter - ctx.ship.heading) * Math.min(ctx.boundaryK * 1.1, 1) * ctx.dt * 1.2
  }
