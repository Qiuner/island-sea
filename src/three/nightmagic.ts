...
export function createPlankton() {
  ...
  return {
    points,
    update(dt: number, nightK: number, dpr: number, sternX: number, sternZ: number, moving: boolean) {
      mat.uniforms.uNight.value = nightK
      mat.uniforms.uDpr.value = dpr
...
export function createJellies() {
  ...
  for (let i = 0; i < 6; i++) {
    ...
    const bell = new THREE.Mesh(new THREE.SphereGeometry(1.7, 14, 9, 0, Math.PI * 2, 0, Math.PI / 2), mat)
    g.add(bell)
    for (let t = 0; t < 6; t++) {
      const ta = (t / 6) * Math.PI * 2
      const tent = new THREE.Mesh(new THREE.CylinderGeometry(0.07, 0.02, 3.4, 4), mat)
    ...
    group.add(g)
    jellies.push({ g, bell, mat, halo, haloMat, phase: rng() * 6.28, cx: 0, cz: 0, a: rng() * 6.28, sp: 0.15 + rng() * 0.2 })
  }
  ...
  return {
    group,
    update(sim: number, dt: number, nightK: number, shipX: number, shipZ: number) {
      ...
      for (const j of jellies) {
        ...
        if (Math.hypot(j.cx - shipX, j.cz - shipZ) > 260) place(j, shipX, shipZ)
        const pulse = 0.5 + 0.5 * Math.sin(sim * 1.6 + j.phase)
        j.g.position.set(j.cx, 0.6 + pulse * 0.6, j.cz)
        j.g.scale.set(1.4 * (0.9 + pulse * 0.2), 1.4 * (1.1 - pulse * 0.2), 1.4 * (0.9 + pulse * 0.2))
