...
export class Ship {
  ...
  constructor() {
    ...
    const cream = new THREE.MeshLambertMaterial({ color: '#efdcb4', flatShading: true })
    const darkWood = new THREE.MeshLambertMaterial({ color: '#6e4a2a', flatShading: true })
    const midWood = new THREE.MeshLambertMaterial({ color: '#9a6a3c', flatShading: true })
    ...
    const mkHullLayer = (w: number, depth: number, mat: THREE.Material): THREE.Mesh => {
      const g = new THREE.ExtrudeGeometry(outline(w), { depth, bevelEnabled: false })
    ...
    const deck = mkHullLayer(0.82, 0.22, new THREE.MeshLambertMaterial({ color: '#e8d4b0', flatShading: true }))
    deck.position.y = 1.1
    this.group.add(hull, strake, rubrail, deck)

    // 艏柱与船尾舵
    const stem = new THREE.Mesh(new THREE.BoxGeometry(0.16, 1.1, 0.34), darkWood)
    stem.position.set(0, 1.45, 3.5)
    stem.rotation.x = 0.18
    const rudder = new THREE.Mesh(new THREE.BoxGeometry(0.14, 1.3, 0.55), darkWood)
    rudder.position.set(0, 0.45, -2.9)
    const tiller = new THREE.Mesh(new THREE.CylinderGeometry(0.05, 0.07, 1.1, 5), midWood)
    tiller.position.set(0, 1.45, -2.35)
    tiller.rotation.x = -0.9
    this.group.add(stem, rudder, tiller)

    // ---- 桅杆 / 帆桁 / 条纹帆 / 三角旗 ----
    const mast = new THREE.Mesh(new THREE.CylinderGeometry(0.14, 0.18, 6.4, 7), midWood)
    mast.position.set(0, 4.1, -0.5)
    const boom = new THREE.Mesh(new THREE.CylinderGeometry(0.08, 0.08, 3.5, 6), midWood)
    ...
    this.pennant = new THREE.Mesh(
      ...
      new THREE.MeshLambertMaterial({ color: '#ff6b57', emissive: '#ff6b57', emissiveIntensity: 0.25, side: THREE.DoubleSide }),
    )
    ...
    const mkRope = (a: THREE.Vector3, b: THREE.Vector3): THREE.Mesh => {
      ...
      const m = new THREE.Mesh(new THREE.CylinderGeometry(0.028, 0.028, len, 4), rope)
    ...
    const barrel = new THREE.Mesh(new THREE.CylinderGeometry(0.42, 0.46, 0.8, 8), midWood)
    barrel.position.set(0.62, 1.6, -1.5)
    const barrelBand = new THREE.Mesh(new THREE.CylinderGeometry(0.45, 0.45, 0.14, 8), new THREE.MeshLambertMaterial({ color: '#c9a14e' }))
    barrelBand.position.set(0.62, 1.62, -1.5)
    const coil = new THREE.Mesh(new THREE.TorusGeometry(0.34, 0.11, 5, 10), rope)
    coil.rotation.x = -Math.PI / 2
    coil.position.set(-0.62, 1.3, -1.7)
    this.group.add(barrel, barrelBand, coil)

    const chest = new THREE.Group()
    const chestBody = new THREE.Mesh(new THREE.BoxGeometry(1.0, 0.62, 0.72), new THREE.MeshLambertMaterial({ color: '#7a4a26', flatShading: true }))
    const chestLid = new THREE.Mesh(new THREE.BoxGeometry(1.0, 0.3, 0.72), new THREE.MeshLambertMaterial({ color: '#8f5a30', flatShading: true }))
    chestLid.position.set(0, 0.42, -0.14)
    chestLid.rotation.x = -0.55
    const band = new THREE.Mesh(
      new THREE.BoxGeometry(1.04, 0.14, 0.76),
      new THREE.MeshLambertMaterial({ color: '#caa14e', emissive: '#ffcf6e', emissiveIntensity: 0.7 }),
    )
    ...
    // 船头灯笼：夜航的暖橙光核
    const lampPole = new THREE.Mesh(new THREE.CylinderGeometry(0.07, 0.09, 1.1, 5), darkWood)
    lampPole.position.set(0, 1.9, 3.05)
    const lampHead = new THREE.Mesh(
      new THREE.OctahedronGeometry(0.3, 0),
      new THREE.MeshLambertMaterial({ color: '#2a2438', emissive: '#ffb84d', emissiveIntensity: 1.6 }),
    )
    ...
    this.group.add(lampPole, lampHead, lampGlow)

    // 船尾小灯：追尾视角里一直能看到的暖光
    const sternLamp = new THREE.Mesh(
      new THREE.OctahedronGeometry(0.22, 0),
      new THREE.MeshLambertMaterial({ color: '#2a2438', emissive: '#ffc25e', emissiveIntensity: 1.5 }),
    )
    ...
    this.group.add(sternLamp, sternGlow)
    ...
    this.arrow = new THREE.Mesh(
      arrowGeo,
      new THREE.MeshBasicMaterial({ color: '#ffa726', transparent: true, opacity: 0 }),
    )
    ...
    for (let i = 0; i < 12; i++) {
      const m = new THREE.Mesh(
        ...
        new THREE.MeshBasicMaterial({ map: wakeTex, color: '#ffffff', transparent: true, opacity: 0, depthWrite: false }),
      )
  ...
  update(dt: number, sim: number, throttle: number, steer: number, speedCap: number): void {
    ...
    const hL = waveHeight(this.pos.x + left.x * 1.6, this.pos.z + left.z * 1.6, sim)
    const hR = waveHeight(this.pos.x - left.x * 1.6, this.pos.z - left.z * 1.6, sim)
