...
export function updateChestDisplay(
  ...
  for (const chest of chestGroup.children) {
    ...
    chest.position.y = chest.userData.baseY + Math.sin(sim * 1.6 + chest.userData.bob) * 0.7
...
export function updateHighlight(
  ...
  const nextHighlightK = highlightK + (wantHighlight - highlightK) * (1 - Math.exp(-5 * dt))
...
export function updateSmoke(
  ...
  for (const particle of smoke) {
    ...
    ;(particle.spr.material as THREE.SpriteMaterial).opacity = Math.min(particle.life * 1.4, 0.6)
  }
}
...
export function updateLantern(
  ...
  const nextLanternOn = lanternOn + (wantLantern - lanternOn) * (1 - Math.exp(-3 * dt))
...
export function updateFogSprites(
  ...
  for (const fog of fogSprites) {
    ...
    material.opacity += (fogOpacity - material.opacity) * (1 - Math.exp(-2.5 * dt))
  }
}
...
export function updateBeacon(
  ...
  material.opacity += (targetOpacity - material.opacity) * (1 - Math.exp(-6 * dt))
