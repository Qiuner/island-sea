...
export interface MaterialFactory {
  (hex: string, emissiveHex?: string, emissiveIntensity?: number): THREE.MeshLambertMaterial
}
...
export function buildIslandGeometry(ctx: IslandGeometryContext): number {
  const { def, group, radius: r, salt, rng, mk, mkTree, jitter, applyGrad } = ctx
  ...
  if (def.theme === 'forest') {
    ...
    if (roll < 0.3) {
      ...
      for (let i = 0; i < n; i++) {
        ...
        const gy = 1.0 + dh * Math.sqrt(Math.max(0, 1 - (rr / domeRadius) * (rr / domeRadius))) * 0.96
      ...
      for (const [ox, oz, domeRadius, dh] of spots) {
        const domeGeo = new THREE.SphereGeometry(domeRadius, 10, 7, 0, Math.PI * 2, 0, Math.PI / 2)
        domeGeo.scale(1, dh / domeRadius, 1)
        ...
        group.add(dome)
        mkTree(ox + domeRadius * 0.1, oz - domeRadius * 0.1, 1 + dh * 0.9, 1.2 + rng() * 0.7)
        mkTree(ox - domeRadius * 0.35, oz + domeRadius * 0.3, 1 + dh * 0.55, 0.9 + rng() * 0.6)
      }
      topY = r * 0.42 + 5
    } else if (roll < 0.8) {
    ...
    for (let i = 0; i < 3; i++) {
      ...
      const to = new THREE.Vector3(Math.cos(a) * r * scale, mh * 0.3, Math.sin(a) * r * scale)
