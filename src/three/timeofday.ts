...
import { DAY, DUSK, NIGHT, DAWN, blendThemes, cloneTheme, type ThemeVals } from './themes'
import { DAY_CYCLE_SEC } from './config'
import { smoothstep, easeInOutQuad } from './ease'
...
const TOD_KEYS: { t: number; th: ThemeVals; night: number; label: string; tint: [number, number, number]; tintAmt: number }[] = [
  { t: 0.0, th: DAY, night: 0.0, label: '白天', tint: [1, 1, 1], tintAmt: 0 },
  { t: 0.34, th: DUSK, night: 0.4, label: '黄昏', tint: [1.14, 1.0, 0.8], tintAmt: 0.55 },
  { t: 0.5, th: NIGHT, night: 1.0, label: '夜晚', tint: [0.82, 0.9, 1.2], tintAmt: 0.5 },
  { t: 0.84, th: DAWN, night: 0.22, label: '黎明', tint: [1.1, 1.0, 0.92], tintAmt: 0.42 },
