...
import { DOLPHIN_SCALE, WHALE_SCALE, CREATURE_BOUND } from './config'

// 海洋生物：海鸥(绕船飞) / 海豚(跃水+水下阴影) / 鲸(拱背喷水+阴影预告) / 鱼群(暗色群影)。
// 原先散在 world.ts 的构造/place/update，整体内聚到这里；World 只 new 一个、每帧 update() 一行。

type Bird = { g: THREE.Group; wl: THREE.Mesh; wr: THREE.Mesh; c: THREE.Vector3; r: number; sp: number; ph: number }
type Dolphin = { g: THREE.Group; shadow: THREE.Mesh; nextAt: number; dur: number; cx: number; cz: number; hy: number; height: number; dist: number }
type Whale = { g: THREE.Group; spout: THREE.Mesh; shadow: THREE.Mesh; nextAt: number; dur: number; cx: number; cz: number; hy: number }
type School = { g: THREE.Group; fish: THREE.Mesh[]; cx: number; cz: number; a: number; speed: number }
...
export class CreatureManager {
  ...
  constructor(scene: THREE.Scene) {
    // 海鸥：绕圈扇翅膀的小剪影，天空的"活"
    {
      const wingMat = new THREE.MeshBasicMaterial({ color: '#39405a', side: THREE.DoubleSide })
      ...
      for (const [cx, cy, cz, cr] of spots) {
        ...
        g.add(wl, wr)
        scene.add(g)
        this.birds.push({ g, wl, wr, c: new THREE.Vector3(cx, cy, cz), r: cr, sp: 0.16 + Math.abs(cx % 7) * 0.012, ph: cx })
      }
    }

    // 海豚：定时跃出海面划一道弧再入水（不跃时藏在水下）
    {
      const dolMat = new THREE.MeshLambertMaterial({ color: '#4a5c7e', flatShading: true })
      for (let i = 0; i < 3; i++) {
        const g = new THREE.Group()
        const body = new THREE.Mesh(new THREE.CapsuleGeometry(0.5, 2.0, 5, 9), dolMat)
        body.rotation.x = Math.PI / 2 // 沿 +Z 躺平（+Z 为头）
        body.scale.set(0.82, 0.82, 1.3)
        const dorsal = new THREE.Mesh(new THREE.ConeGeometry(0.32, 0.78, 4), dolMat)
        dorsal.position.set(0, 0.5, 0.05)
        dorsal.rotation.x = -0.25
        const fluke = new THREE.Mesh(new THREE.ConeGeometry(0.55, 0.5, 4), dolMat)
        ...
        const shadow = new THREE.Mesh(
          ...
          new THREE.MeshBasicMaterial({ color: '#08202e', transparent: true, opacity: 0.25, depthWrite: false }),
        )
        shadow.rotation.x = -Math.PI / 2
        shadow.position.y = 0.12
        shadow.visible = false
        scene.add(shadow)
        const d: Dolphin = { g, shadow, nextAt: 3 + i * 3.5, dur: 1.5, cx: 0, cz: 0, hy: 0, height: 2, dist: 8 }
    ...
    {
      const wMat = new THREE.MeshLambertMaterial({ color: '#3b4a63', flatShading: true })
      const g = new THREE.Group()
      ...
      const spout = new THREE.Mesh(
        ...
        new THREE.MeshBasicMaterial({ color: '#eaf6ff', transparent: true, opacity: 0.55, depthWrite: false }),
      )
      ...
      const shadow = new THREE.Mesh(
        ...
        new THREE.MeshBasicMaterial({ color: '#08202e', transparent: true, opacity: 0.3, depthWrite: false }),
      )
      shadow.rotation.x = -Math.PI / 2
      shadow.position.y = 0.12
      shadow.visible = false
      scene.add(shadow)
      this.whale = { g, spout, shadow, nextAt: 9, dur: 5.5, cx: 0, cz: 0, hy: 0 }
      this.placeWhale(0, 0)
    }

    // 水下鱼群：一簇暗色小鱼贴着水面漂动（不透明海面下，用暗色群影表现"水下鱼群"）
    {
      const fMat = new THREE.MeshBasicMaterial({ color: '#213141', transparent: true, opacity: 0.5, depthWrite: false })
      for (let s = 0; s < 3; s++) {
        const g = new THREE.Group()
        ...
        for (let i = 0; i < 14; i++) {
          ...
          f.position.set((i % 5) * 1.8 - 3.6 + Math.sin(i) * 1.2, 0, Math.floor(i / 5) * 2.2 - 3)
          g.add(f)
          fish.push(f)
        }
        g.position.y = 0.45
        scene.add(g)
        const sc: School = { g, fish, cx: 0, cz: 0, a: (s / 3) * 6.28, speed: 0.35 + s * 0.12 }
  ...
  update(sim: number, dt: number, shipX: number, shipZ: number): void {
    ...
    for (const d of this.dolphins) {
      ...
      ;(d.shadow.material as THREE.MeshBasicMaterial).opacity = 0.28 * (1 - THREE.MathUtils.clamp(dy / 3, 0, 1))
    }
    // 鲸鱼：缓缓拱背出水 + 中段喷水
    {
      const wl = this.whale
      ...
      if (k < 0 || k > 1) {
        ...
        ;(wl.shadow.material as THREE.MeshBasicMaterial).opacity = 0.1 + 0.28 * near
      }
    }
    ...
    for (const sc of this.schools) {
      ...
      if (Math.hypot(sc.cx - shipX, sc.cz - shipZ) > 250) this.placeSchool(sc, shipX, shipZ)
      sc.g.position.set(sc.cx, 0.45, sc.cz)
      sc.g.rotation.y = -sc.a
      for (let i = 0; i < sc.fish.length; i++) sc.fish[i].rotation.z = Math.sin(sim * 6 + i * 0.7) * 0.35
    }
  }
}
