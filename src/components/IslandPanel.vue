...
onMounted(scheduleCarousel)
onBeforeUnmount(clearCarouselTimer)
...
function advanceCarousel(offset: number) {
  ...
  carouselIndex.value = (carouselIndex.value + offset + photos.length) % photos.length
}
...
function movePhoto(offset: number) {
  const photos = island.value?.photos ?? []
  if (photos.length < 2) return
  const nextIndex = (activePhotoIndex.value + offset + photos.length) % photos.length
  ...
  >
    <article class="island-dossier">
      ...
      >
        <div class="gallery-heading">
          <div>
            <p class="gallery-eyebrow">训练营影像</p>
            <h3 id="island-photos-title">{{ island.builder }}的造物时刻</h3>
          </div>
        ...
        <div class="gallery-footer">
          ...
          <div v-if="island.photos.length > 1" class="gallery-controls">
            <button type="button" aria-label="上一张照片" @click="moveCarousel(-1)">←</button>
            ...
            <button type="button" aria-label="下一张照片" @click="moveCarousel(1)">→</button>
          </div>
        </div>
      </section>
      ...
      <section class="island-details">
        <header class="dossier-header">
          <p class="builder">{{ island.builder }} 的成长岛</p>
          <h2>{{ island.name }}</h2>
        ...
        <div class="dossier-content">
          ...
          <div class="section-heading">
            <h3 id="island-works-title">孩子的作品</h3>
            <span>{{ island.projects.length }} 件</span>
          </div>
          <div v-if="island.projects.length" class="projects">
            ...
            >
              ...
              <span class="project-open">查看作品 <span aria-hidden="true">↗</span></span>
            </button>
          </div>
    ...
    >
      <div v-if="activePhoto" class="photo-viewer-content">
        <button class="viewer-close" type="button" aria-label="关闭照片" @click="closePhoto">×</button>
        <button
