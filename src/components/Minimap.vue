...
import { WORLD_RADIUS } from '../data/islands'

// 小地图：独立 2D canvas，每帧直接读 world（不走 Vue 响应式，避免每帧重渲染）
...
function draw() {
  ...
  for (let i = 0; i < 24; i++) {
    ...
    ctx.lineTo(c + Math.cos(ang) * (MAP_R - 1.5), c + Math.sin(ang) * (MAP_R - 1.5))
    ctx.stroke()
  }
  ...
  for (const isl of world.minimapData()) {
    ...
    if (isl.status === 'foggy') {
      ctx.fillStyle = night ? 'rgba(150,160,190,0.4)' : 'rgba(240,245,250,0.42)'
      ctx.beginPath()
...
<template>
  <div class="minimap">
    <canvas ref="cv" :style="{ width: SIZE + 'px', height: SIZE + 'px' }" @click="onClick"></canvas>
  </div>
</template>
